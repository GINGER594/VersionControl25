...
import ("os"
        "os/exec")
func fmapBashCommand(bashCmd, dir string, args []string) { //applies bash command as a functor to dir (other args optional)
...
func main() {
    if verDir := `./.vc25/`; len(os.Args) == 3 { //defining path to version backup folder
        ...
        if os.Args[1] == "cpver" { //copy version (creates new if not exists)
            ...
            fmapBashCommand("rm", verDir+os.Args[2]+`/`, []string{"-r"}) //removing entities from the specified version dir
            fmapBashCommand("cp", `./`, []string{verDir+os.Args[2]+`/`, "-r"}) //copying entities from the current dir to the specified version dir
            ...
            fmapBashCommand("rm", `./`, []string{"-r"}) //removing entities from the current dir
            fmapBashCommand("cp", verDir+os.Args[2]+`/`, []string{`./`, "-r"}) //copying entities from the specified version dir to the current dir
        }
    }
}
